  ...
  useEffect(() => {
    if (visible && triggerRef.current) {
      ...
      setCoords({
        ...
        top: rect.top + window.scrollY - 8 // El -8 le da una separación hacia arriba
      });
    }
  }, [visible]);
      ...
      <div className="p-4 flex flex-col gap-4">
        <div className="grid grid-cols-[1fr_1fr_80px] gap-3">
          <div className="flex flex-col gap-1">
            ...
            <input type="text" className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:bg-white focus:border-navy focus:ring-4 focus:ring-navy/5 outline-none transition-all" value={obs.participant || ''} onChange={e => onLocalChange(obs.id!, { participant: e.target.value })} onBlur={e => onAction(() => onSave(obs.id!, { participant: e.target.value }))} placeholder="P1" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          <AutoGrowTextarea className="w-full p-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:bg-white focus:border-navy focus:ring-4 focus:ring-navy/5 outline-none transition-all font-medium" value={obs.comments || ''} onChange={e => onLocalChange(obs.id!, { comments: e.target.value })} onBlur={e => onAction(() => onSave(obs.id!, { comments: e.target.value }))} placeholder="Ej. Dudó..." rows={2} />
        </div>

        <div className="flex flex-col gap-1">
          <label className="font-black text-[0.7rem] text-slate-500 uppercase tracking-widest text-red-800">Problema detectado</label>
          <AutoGrowTextarea className="w-full p-2.5 border border-red-100 rounded-lg text-sm bg-red-50/30 focus:bg-white focus:border-red-400 focus:ring-4 focus:ring-red-50 outline-none transition-all font-medium italic" value={obs.problem || ''} onChange={e => onLocalChange(obs.id!, { problem: e.target.value })} onBlur={e => onAction(() => onSave(obs.id!, { problem: e.target.value }))} placeholder="Ej. Nombre..." rows={2} />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <AutoGrowTextarea className="w-full p-2.5 border border-green-100 rounded-lg text-sm bg-green-50/30 focus:bg-white focus:border-green-400 focus:ring-4 focus:ring-green-50 outline-none transition-all font-medium" value={obs.proposal || ''} onChange={e => onLocalChange(obs.id!, { proposal: e.target.value })} onBlur={e => onAction(() => onSave(obs.id!, { proposal: e.target.value }))} placeholder="Ej. Cambiar..." rows={2} />
          </div>
        </div>
        ...
        <div className="flex justify-end pt-2 border-t border-slate-100 mt-2">
          {confirmDelete ? (
            ...
            <button
              type="button"
              className="inline-flex items-center gap-1.5 bg-transparent border-none text-slate-400 cursor-pointer p-2 rounded-lg transition-all hover:bg-red-50 hover:text-red-600"
  ...
  return (
    <tr className="hover:bg-slate-50/50 transition-colors border-b border-slate-100 last:border-0">
      <td className="p-2 border-r border-slate-100">
        <input type="text" className="w-full p-2 border border-transparent bg-transparent rounded-lg text-sm transition-all focus:bg-white focus:border-navy outline-none font-bold" value={obs.participant || ''} onChange={e => handleLocalChange(obs.id!, { participant: e.target.value })} onBlur={() => handleActionWithStatus(() => onSave(obs.id!, { participant: obs.participant }))} placeholder="P1" />
      </td>
      <td className="p-2 border-r border-slate-100">
        <input type="text" className="w-full p-2 border border-transparent bg-transparent rounded-lg text-sm transition-all focus:bg-white focus:border-navy outline-none font-medium" value={obs.profile || ''} onChange={e => handleLocalChange(obs.id!, { profile: e.target.value })} onBlur={() => handleActionWithStatus(() => onSave(obs.id!, { profile: obs.profile }))} placeholder="Perfil" />
      </td>
      <td className="p-2 text-center border-r border-slate-100">
        <input type="text" className="w-full p-2 border border-transparent bg-transparent rounded-lg text-sm text-center transition-all focus:bg-white focus:border-navy outline-none font-mono font-bold" value={obs.task_ref || ''} onChange={e => handleLocalChange(obs.id!, { task_ref: e.target.value })} onBlur={() => handleActionWithStatus(() => onSave(obs.id!, { task_ref: obs.task_ref }))} placeholder="T1" />
      </td>
      <td className="p-2 text-center border-r border-slate-100">
        <select
          className={`w-full p-2 border ${okStyle.border} rounded-lg text-[0.75rem] ${okStyle.bg} ${okStyle.text} font-black outline-none cursor-pointer shadow-sm`}
          value={obs.success_level}
          onChange={e => {
            const val = e.target.value as SuccessStatus;
      ...
      <td className="p-2 border-r border-slate-100">
        <AutoGrowTextarea className="w-full p-2 border border-transparent bg-transparent rounded-lg text-sm transition-all focus:bg-white focus:border-navy outline-none font-medium resize-none" value={obs.comments || ''} onChange={e => handleLocalChange(obs.id!, { comments: e.target.value })} onBlur={() => handleActionWithStatus(() => onSave(obs.id!, { comments: obs.comments }))} placeholder="Comentarios..." rows={1} />
      </td>
      <td className="p-2 border-r border-slate-100">
        <AutoGrowTextarea className="w-full p-2 border border-transparent bg-transparent rounded-lg text-sm transition-all focus:bg-white focus:border-red-400 outline-none font-medium italic text-red-900 resize-none" value={obs.problem || ''} onChange={e => handleLocalChange(obs.id!, { problem: e.target.value })} onBlur={() => handleActionWithStatus(() => onSave(obs.id!, { problem: obs.problem }))} placeholder="Problema..." rows={1} />
      </td>
      <td className="p-2 text-center border-r border-slate-100">
        <select
          className={`w-full p-2 border ${sStyle.border} rounded-lg text-[0.75rem] ${sStyle.bg} ${sStyle.text} font-black outline-none cursor-pointer shadow-sm`}
          value={obs.severity}
          onChange={e => {
            const val = e.target.value as Severity;
      ...
      <td className="p-2 border-r border-slate-100">
        <AutoGrowTextarea className="w-full p-2 border border-transparent bg-transparent rounded-lg text-sm transition-all focus:bg-white focus:border-green-400 outline-none font-medium text-green-900 resize-none" value={obs.proposal || ''} onChange={e => handleLocalChange(obs.id!, { proposal: e.target.value })} onBlur={() => handleActionWithStatus(() => onSave(obs.id!, { proposal: obs.proposal }))} placeholder="Mejora..." rows={1} />
      </td>
